...
class AtlassianMCP(FastMCP[MainAppContext]):
    ...
    def http_app(
        ...
        # Get the base app first
        app = super().http_app(path=path, middleware=middleware, transport=transport)

        # Wrap the app with our custom ASGI middleware
        wrapped_app = UserTokenMiddleware(app, mcp_server_ref=self)
...
class UserTokenMiddleware:
    """ASGI middleware to extract Atlassian user tokens/credentials from Authorization headers."""

    def __init__(self, app: Any, mcp_server_ref: Optional["AtlassianMCP"] = None) -> None:
        self.app = app
    ...
    async def __call__(self, scope, receive, send):
        ...
        logger.debug(
            f"UserTokenMiddleware: Comparing request_path='{request_path}' with mcp_path='{mcp_path}'. Request method='{request.method}'"
        )

        # Check if this is an MCP POST request that needs authentication
        if request_path == mcp_path and request.method == "POST":
            ...
            # Check for authentication errors and send error response directly
            error_response = None
            ...
            if auth_header and auth_header.startswith("Bearer "):
                ...
                if not token:
                    ...
                    # Store auth info in scope for downstream handlers
                    ...
                    logger.debug(
                        f"UserTokenMiddleware: Set scope state (pre-validation): "
                        f"auth_type='oauth', token_present=True"
                    )
                ...
                if not token:
                    ...
                    scope["state"]["user_atlassian_email"] = None
                    logger.debug(
                        "UserTokenMiddleware: Set scope state for PAT auth."
                    )
            elif auth_header:
                logger.warning(
            ...
            # Handle cloudId extraction
            if cloud_id_header and cloud_id_header.strip():
                scope.setdefault("state", {})
                scope["state"]["user_atlassian_cloud_id"] = cloud_id_header.strip()
                logger.debug(
                    f"UserTokenMiddleware: Extracted cloudId from header: {cloud_id_header.strip()}"
                )
            else:
                scope.setdefault("state", {})
                scope["state"]["user_atlassian_cloud_id"] = None
                logger.debug(
            ...
            # Send error response if authentication failed
            if error_response:
